...
namespace
{
    ...
    inline constexpr f32 FovY = std::numbers::pi_v<f32> / 3;
    inline constexpr f32 NearZ = 0.1f, FarZ = 300.0f;

    Frustum3D DefaultFrustum(Vec3 eye = zeros()) {
        const auto view = viewMat(eye, unitQuat());
        const auto proj = perspective(FovY, 1.0f, NearZ, FarZ);

        return makeFrustum3D(proj * view);
    }

    AABB3D UnitBoxAt(Vec3 center) {
        return AABB3D{.center = center, .halfScale = 0.5f * ones()};
    }
}
...
TEST(Frustum3D, KeepsWhatIsInFront) {
    ...
    EXPECT_TRUE(
        OverlapFrustumAABB3D(frustum, UnitBoxAt({0.0f, 0.0f, NearZ * 2}))
    );
}
...
TEST(Frustum3D, DropsWhatIsBehind) {
    ...
    EXPECT_FALSE(
        OverlapFrustumAABB3D(frustum, UnitBoxAt({0.0f, 0.0f, -10.0f}))
    );
...
TEST(Frustum3D, DropsWhatIsBeyondFar) {
    ...
    EXPECT_FALSE(
        OverlapFrustumAABB3D(frustum, UnitBoxAt({0.0f, 0.0f, FarZ + 10.0f}))
    );
}
...
TEST(Frustum3D, DropsWhatIsOutsideTheCone) {
    ...
    EXPECT_FALSE(
        OverlapFrustumAABB3D(frustum, UnitBoxAt({0.0f, -8.0f, 10.0f}))
    );
}
...
TEST(Frustum3D, KeepsBoxesStraddlingAPlane) {
    ...
    EXPECT_TRUE(OverlapFrustumAABB3D(
        frustum,
        AABB3D{.center = {12.0f, 0.0f, 10.0f}, .halfScale = {8.0f, 1.0f, 1.0f}}
    ));
    // and one that swallows the whole frustum
    EXPECT_TRUE(OverlapFrustumAABB3D(
        frustum,
        AABB3D{.center = zeros(), .halfScale = 1000.0f * ones()}
    ));
}
...
TEST(Frustum3D, FollowsTheEye) {
    ...
    EXPECT_FALSE(
        OverlapFrustumAABB3D(DefaultFrustum({0.0f, 0.0f, 20.0f}), box)
    );
}
